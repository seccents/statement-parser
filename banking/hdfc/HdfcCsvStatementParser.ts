...
export class HdfcCsvStatementParser {
	public static parseHDFCAccountStatement(): Promise<BankTransaction[]> {
		return new Promise<BankTransaction[]>((resolve): void => {
			...
			const csvContent = readFileSync(pathResolve(__dirname, '../../../assets', 'banking/hdfc-lakshya.txt'))
				.toString()
				.split('\n'); // read file and convert to array by line break
			if (csvContent[0].trim() == '') {
				csvContent.shift(); // remove the the first element from array
			}
			writeFileSync(pathResolve(__dirname, '../../../assets', 'banking/hdfc-lakshya.csv'), csvContent.join('\n')); // convert array back to string

			createReadStream(pathResolve(__dirname, '../../../assets', 'banking/hdfc-lakshya.csv'))
